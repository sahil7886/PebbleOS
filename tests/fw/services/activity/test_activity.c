...
// ---------------------------------------------------------------------------------------
// Nearby automatic sessions of the same type should become one continuous workout. The
// detector's temporary ongoing entry must disappear when the newer session is finalized.
void test_activity__nearby_automatic_sessions_merge(void) {
  const int first_len_min = 20;
  const int second_len_min = 15;
  ActivitySession first = {
      .start_utc = 1000,
      .length_min = first_len_min,
      .type = ActivitySessionType_Run,
      .step_data =
          {
              .steps = 2000,
              .active_kcalories = 120,
              .resting_kcalories = 20,
              .distance_meters = 4000,
          },
  };
  ActivitySession second = {
      .start_utc =
          first.start_utc + (first_len_min + ACTIVITY_SESSION_MERGE_GAP_MIN) * SECONDS_PER_MINUTE,
      .length_min = second_len_min,
      .type = ActivitySessionType_Run,
      .ongoing = true,
      .step_data =
          {
              .steps = 1500,
              .active_kcalories = 90,
              .resting_kcalories = 15,
              .distance_meters = 3000,
          },
  };

  activity_sessions_prv_add_activity_session(&first);
  activity_sessions_prv_add_activity_session(&second);
  cl_assert_equal_i(activity_private_state()->activity_sessions_count, 2);

  second.ongoing = false;
  activity_sessions_prv_add_activity_session(&second);

  ActivitySession expected = first;
  expected.length_min = first_len_min + ACTIVITY_SESSION_MERGE_GAP_MIN + second_len_min;
  expected.step_data.steps += second.step_data.steps;
  expected.step_data.active_kcalories += second.step_data.active_kcalories;
  expected.step_data.resting_kcalories += second.step_data.resting_kcalories;
  expected.step_data.distance_meters += second.step_data.distance_meters;
  cl_assert_equal_i(activity_private_state()->activity_sessions_count, 1);
  cl_assert_equal_m(&activity_private_state()->activity_sessions[0], &expected,
                    sizeof(ActivitySession));

  ActivitySession later = second;
  later.start_utc = expected.start_utc + expected.length_min * SECONDS_PER_MINUTE +
                    (ACTIVITY_SESSION_MERGE_GAP_MIN + 1) * SECONDS_PER_MINUTE;
  activity_sessions_prv_add_activity_session(&later);
  cl_assert_equal_i(activity_private_state()->activity_sessions_count, 2);
}
